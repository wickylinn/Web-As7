...
function createJQuerySections(){
  ...
  wrap.innerHTML=`
    ...
    <div class="jquery-section" style="background:linear-gradient(135deg,var(--bg1-color) 0%,var(--bg-color) 100%);border-radius:20px;padding:35px;margin:30px auto;max-width:900px;box-shadow:0 10px 40px rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.1)">
      ...
      <h3 style="color:var(--text-color);margin:0 0 15px;font-size:18px">API Code Examples</h3>
      <div style="position:relative;background:#1a1a2e;padding:20px;border-radius:12px;margin-bottom:15px;border:1px solid rgba(255,255,255,.1)">
        <button class="jquery-copy-btn" data-target="#jquery-code1" style="position:absolute;top:10px;right:10px;background:var(--accent-color);color:#fff;border:none;padding:8px 16px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;transition:.3s">📋 Copy</button>
        <pre id="jquery-code1" style="margin:0;color:#00f7ff;font-family:'Courier New',monospace;font-size:14px;overflow-x:auto">fetch('/api/playlists')
  .then(res=>res.json())
  .then(data=>console.log(data));</pre>
      </div>
      <div style="position:relative;background:#1a1a2e;padding:20px;border-radius:12px;margin-bottom:25px;border:1px solid rgba(255,255,255,.1)">
        <button class="jquery-copy-btn" data-target="#jquery-code2" style="position:absolute;top:10px;right:10px;background:var(--accent-color);color:#fff;border:none;padding:8px 16px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;transition:.3s">📋 Copy</button>
        <pre id="jquery-code2" style="margin:0;color:#53c52a;font-family:'Courier New',monospace;font-size:14px;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;overflow-x:hidden">
        const addSong=async(songId)=>{await fetch('/api/songs/add',{method:'POST',body:JSON.stringify({songId})})};
        </pre>
      </div>
